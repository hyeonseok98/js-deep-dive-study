...
### ✅ 39.6.6 노드 이동
> DOM에 이미 존재하는 노드를 appendChild 또는 insertBefore 메서드를 사용하면 DOM에 다시 추가하면<br>
...
### ✅ 39.6.7 노드 복사 
> Node.prototype.clibeNode([deep : true | false]) 메서드는 노드의 사본을 생성하여 반환한다. <br>
...
#### data 어트리뷰트
- 정의: HTML 요소에 사용자 정의 데이터를 저장할 수 있는 어트리뷰트입니다.
- 형식: data- 접두사 + 사용자 정의 이름 = "값"
- 예:` <li data-user-id="1" data-sex="male">John</li>`

#### dataset 프로퍼티
- 정의: JavaScript에서 data 어트리뷰트에 접근하기 위한 프로퍼티입니다.
- 반환값: 모든 data 어트리뷰트를 담은 DOMStringMap 객체를 반환합니다.
- 이름 변환: data- 접두사 다음의 이름을 카멜 케이스로 변환합니다.
...
| `replace(oldClassName, newClassName)`| 기존 클래스를 새로운 클래스로 교체                                         | `element.classList.replace('oldClass', 'newClass')`                |
| `toggle(className, [force])`        | 클래스가 있으면 삭제, 없으면 추가. `force` 인수로 강제 추가/삭제 가능        | `element.classList.toggle('class1')`<br>`element.classList.toggle('class1', true)` |
