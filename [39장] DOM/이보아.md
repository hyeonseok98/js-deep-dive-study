...
- 깊은 복사 (deep copy) : `cloneNode(true)`를 사용하여 노드를 깊게 복사합니다. 깊은 복사는 노드 자신과 모든 자식 노드를 복사한다.
...
2. CSSStyleDeclaration 객체: JavaScript에서는 프로퍼티 이름을 카멜케이스로 사용 (예) backgroundColor)프로퍼티에 값을 할당하면 인라인 스타일로 추가다. 
